...
`DataSource` requires a <<sparkSession, SparkSession>> and a <<className, fully-qualified class name of the data source provider>> to be created (among <<creating-instance, other optional parameters>>). `DataSource` uses them to <<lookupDataSource, lookupDataSource>> (for the <<providingClass, providingClass>>). Once created, `DataSource` is used to <<resolveRelation, resolve a relation>> (the xref:spark-sql-BaseRelation.adoc[BaseRelation] that represents the external data source in logical plans with xref:spark-sql-LogicalPlan-LogicalRelation.adoc[LogicalRelation] leaf logical operator).
...
* [[className]] Fully-qualified class name of the data source provider (aka _data source format_)
...
=== [[lookupDataSource]] Looking Up Java Class For Data Source Provider -- `lookupDataSource` Utility
...
`lookupDataSource` looks up the Java's https://docs.oracle.com/javase/8/docs/api/java/lang/Class.html[Class] for the given `provider` in the <<backwardCompatibilityMap, backwardCompatibilityMap>> first and then replaces the class name exclusively for the `orc` provider per link:spark-sql-properties.adoc#spark.sql.orc.impl[spark.sql.orc.impl] internal configuration property:
