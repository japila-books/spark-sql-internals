...
[NOTE]
====
`HiveTableRelation` is removed from a logical plan when `HiveAnalysis` rule is executed (and transforms a `InsertIntoTable` with `HiveTableRelation` to an link:spark-sql-LogicalPlan-InsertIntoHiveTable.adoc[InsertIntoHiveTable]).
