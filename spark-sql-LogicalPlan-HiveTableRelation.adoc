...
* `HiveMetastoreCatalog` is requested to link:spark-sql-HiveMetastoreCatalog.adoc#convertToLogicalRelation[convert a HiveTableRelation to a LogicalRelation]
...
* `HiveTableScanExec` physical operator is link:spark-sql-SparkPlan-HiveTableScanExec.adoc#doExecute[executed]
====
