...
1. `CommandUtils` is requested to link:spark-sql-CommandUtils.adoc#updateTableStats[updateTableStats]
...
| [[spark.sql.statistics.size.autoUpdate.enabled]] `spark.sql.statistics.size.autoUpdate.enabled`
| `false`
a| Enables automatic update for table size once table's data has been changed.
