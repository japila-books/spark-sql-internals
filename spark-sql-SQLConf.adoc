...
a| [[caseSensitiveAnalysis]] Used in:
...
* `AstBuilder` is requested to <<spark-sql-AstBuilder.adoc#visitDereference, visitDereference>> and <<spark-sql-AstBuilder.adoc#visitColumnReference, visitColumnReference>>
...
| [[windowExecBufferSpillThreshold]] Used exclusively when `WindowExec` unary physical operator is link:spark-sql-SparkPlan-WindowExec.adoc#doExecute[executed].
