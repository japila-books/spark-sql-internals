...
| [[CONVERT_METASTORE_ORC]] Used when `RelationConversions` logical post-hoc evaluation rule is executed (and requested to link:spark-sql-Analyzer-RelationConversions.adoc#isConvertible[isConvertible])
...
| [[CONVERT_METASTORE_PARQUET]] Used when `RelationConversions` logical post-hoc evaluation rule is executed (and requested to link:spark-sql-Analyzer-RelationConversions.adoc#isConvertible[isConvertible])
...
| link:spark-sql-properties.adoc#spark.sql.hive.manageFilesourcePartitions[spark.sql.hive.manageFilesourcePartitions]
...
a| [[metastorePartitionPruning]][[HIVE_METASTORE_PARTITION_PRUNING]] Used when link:spark-sql-SparkPlan-HiveTableScanExec.adoc[HiveTableScanExec] physical operator is executed with a partitioned table (and requested for link:spark-sql-SparkPlan-HiveTableScanExec.adoc#rawPartitions[rawPartitions])
