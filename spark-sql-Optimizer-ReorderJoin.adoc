== [[ReorderJoin]] ReorderJoin Logical Query Optimization -- Reordering Inner and Cross Joins
...
import org.apache.spark.sql.catalyst.dsl.expressions._
import org.apache.spark.sql.catalyst.dsl.plans._
val plan1 = LocalRelation('id.long, 'v1.long).as("t1")
val plan2 = LocalRelation('id.long, 'v2.long).as("t2")
val plan3 = LocalRelation('id.long, 'v3.long).as("t3")
val plan4 = LocalRelation('id.long, 'v4.long).as("t4")
...
val t1_id = plan1.outputSet.head
val t2_id = plan2.outputSet.head
val t1_v1 = plan1.outputSet.tail.head
val t4_v4 = plan4.outputSet.tail.head
val c1 = t1_id === t2_id
val c2 = t1_v1 === t4_v4
...
assert(!c1.references.subsetOf(plan1.outputSet))
assert(!c1.references.subsetOf(plan2.outputSet))
...
scala> println(plan.numberedTreeString)
00 Join Cross
01 :- Join Inner, (v1#1L = v4#7L)
02 :  :- Join Inner, (id#0L = id#2L)
03 :  :  :- SubqueryAlias t1
04 :  :  :  +- LocalRelation <empty>, [id#0L, v1#1L]
05 :  :  +- SubqueryAlias t2
06 :  :     +- LocalRelation <empty>, [id#2L, v2#3L]
07 :  +- SubqueryAlias t4
08 :     +- LocalRelation <empty>, [id#6L, v4#7L]
09 +- SubqueryAlias t3
10    +- LocalRelation <empty>, [id#4L, v3#5L]
----
