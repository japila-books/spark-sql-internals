== [[ReorderJoin]] ReorderJoin Logical Optimization Rule -- Reordering Inner and Cross Joins
...
`ReorderJoin` <<apply, applies>> the join optimizations on a logical plan with 2 or more inner and cross joins with one or more join conditions (possibly separated by `Filter` operators).
