...
Cost-Based Optimization uses <<optimizations, custom logical optimization rules>> that transform the logical plan of a structured query based on the cost statistics.

=== [[statistics]] Cost Statistics

The cost statistics of a table are as follows:

1. [[total-size-stat]] link:spark-sql-LogicalPlan-AnalyzeTableCommand.adoc#total-size-stat[Total size]
1. [[row-count-stat]] link:spark-sql-LogicalPlan-AnalyzeTableCommand.adoc#row-count-stat[Row count]
