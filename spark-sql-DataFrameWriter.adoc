...
=== [[save]] Writing Result of Structured Query (DataFrame) to Data Source -- `save` Method
...
`save` writes the result of a structured query (a <<spark-sql-Dataset.adoc#, Dataset>>) to a data source.
...
=== [[saveToV1Source]] `saveToV1Source` Internal Method
...
`saveToV1Source`...FIXME

NOTE: `saveToV1Source` is used exclusively when `DataFrameWriter` is requested to <<save, write the result of a structured query (DataFrame) to data source>> (for <<spark-sql-DataSourceV2.adoc#, DataSourceV2>> with no `WriteSupport` and non-``DataSourceV2`` writers).
