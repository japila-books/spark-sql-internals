...
NOTE: `saveToV1Source` is used exclusively when `DataFrameWriter` is requested to <<save, save>> (for `DataSourceV2` writers with no `WriteSupport` and non-``DataSourceV2`` writers).
