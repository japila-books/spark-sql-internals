...
`DataFrameWriter` is the <<methods, public API>> to write the result of executing a structured query (i.e. the content of a link:spark-sql-Dataset.adoc[Dataset]) to an external storage system in a batch fashion.
...
NOTE: `insertInto` ignores column names and just uses a position-based resolution, i.e. the order of the columns in (the output of) the Dataset matters.

Internally, `insertInto` <<runCommand, executes>> a link:spark-sql-LogicalPlan-InsertIntoTable.adoc[InsertIntoTable] logical operator (that submits a Spark job).
