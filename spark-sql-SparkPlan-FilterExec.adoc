...
In the end, `doExecute` requests the <<child, child>> physical operator to <<spark-sql-SparkPlan.adoc#execute, execute>> (that triggers physical query planning and generates an `RDD[InternalRow]`) and translates it (that creates a new RDD) by applying a function to each partition with index (using `RDD.mapPartitionsWithIndexInternal`) as follows:
