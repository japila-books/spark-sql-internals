...
`WholeStageCodegenExec` is created exclusively when link:spark-sql-CollapseCodegenStages.adoc[CollapseCodegenStages] physical query optimization rule is executed with link:spark-sql-whole-stage-codegen.adoc#spark.sql.codegen.wholeStage[spark.sql.codegen.wholeStage] configuration property enabled.
