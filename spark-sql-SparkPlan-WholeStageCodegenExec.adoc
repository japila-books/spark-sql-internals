...
[source, scala]
----
val q /* Dataset[_] or DataFrame */ = spark.range(10).where('id === 4)
...
`WholeStageCodegenExec` is <<creating-instance, created>> exclusively when `CollapseCodegenStages` physical query optimization is link:spark-sql-CollapseCodegenStages.adoc#apply[executed] with link:spark-sql-whole-stage-codegen.adoc#spark.sql.codegen.wholeStage[spark.sql.codegen.wholeStage] configuration property enabled.
...
[[child]]
[[codegenStageId]]
`WholeStageCodegenExec` takes a single `child` link:spark-sql-SparkPlan.adoc[physical operator] (a physical subquery tree) and *per-query codegen stage ID* when created.
...
`doExecute` <<doCodeGen, generates the Java source code for the child physical plan subtree>> and link:spark-sql-CodeGenerator.adoc#compile[compiles] it right afterwards.

If all goes well, `doExecute` branches off per the number of link:spark-sql-CodegenSupport.adoc#inputRDDs[input RDDs].
...
`generatedClassName`...FIXME

NOTE: `generatedClassName` is used when...FIXME
