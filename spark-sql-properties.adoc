...
This is for cases not tracked by xref:spark-sql-SQLExecution.adoc[SQL execution], as it happens when a `Dataset` is converted to an RDD either using xref:spark-sql-Dataset.adoc#rdd[rdd] operation or xref:spark-sql-QueryExecution.adoc#toRdd[QueryExecution], and then the returned RDD is used to invoke actions on it.
