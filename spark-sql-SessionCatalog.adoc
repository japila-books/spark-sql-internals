...
* `CreateViewCommand` logical command is requested to <<spark-sql-LogicalPlan-CreateViewCommand.adoc#run, run>> (for a global temporary view)

* `CreateTempViewUsing` logical command is requested to <<spark-sql-LogicalPlan-CreateTempViewUsing.adoc#run, run>> (for a global temporary view)
====
