...
spark.sharedState.externalCatalog.dropTable(db, table = "v1", ignoreIfNotExists = true, purge = true)
...
spark.sharedState.externalCatalog.createTable(tableDef, ignoreIfExists = false)
