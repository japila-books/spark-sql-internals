...
| Boolean mask of columns that are missing (per index)
|===
...
`initBatch` requests `OffHeapColumnVector` or `OnHeapColumnVector` to `allocateColumns` per the input `memMode`, i.e. <<OFF_HEAP, OFF_HEAP>> or <<ON_HEAP, ON_HEAP>> memory modes, respectively. `initBatch` records the allocated column vectors as the internal <<columnVectors, WritableColumnVectors>>.
