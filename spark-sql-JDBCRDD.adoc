...
`JDBCRDD` is <<creating-instance, created>> exclusively when `JDBCRDD` is requested to <<scanTable, scanTable>> (when `JDBCRelation` is requested to link:spark-sql-BaseRelation-JDBCRelation.adoc#buildScan[build a scan]).
...
* [[url]] Connection URL
* [[options]] link:spark-sql-DataFrameReader.adoc#JDBCOptions[JDBCOptions]
