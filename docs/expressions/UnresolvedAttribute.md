...
[[resolved]]
`UnresolvedAttribute` can never be link:expressions/Expression.md#resolved[resolved] (and is replaced at <<analysis-phase, analysis phase>>).
...
Given `UnresolvedAttribute` can never be resolved it should not come as a surprise that it link:expressions/Expression.md#Unevaluable[cannot be evaluated] either (i.e. produce a value given an internal row). When requested to evaluate, `UnresolvedAttribute` simply reports a `UnsupportedOperationException`.
