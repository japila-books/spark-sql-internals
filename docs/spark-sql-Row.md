...
!!! note
    It is [RowEncoder](RowEncoder.md) to take care of assigning a schema to a `Row` when `toDF` on a [Dataset](Dataset.md) or when instantiating [DataFrame](spark-sql-DataFrame.md) through [DataFrameReader](DataFrameReader.md).
