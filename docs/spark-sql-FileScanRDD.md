...
`FileScanRDD` is an `RDD` of spark-sql-InternalRow.md[internal binary rows] (i.e. `RDD[InternalRow]`) that is the input RDD of a spark-sql-SparkPlan-FileSourceScanExec.md[FileSourceScanExec] physical operator (in <<spark-sql-whole-stage-codegen.md#, Whole-Stage Java Code Generation>>).

`FileScanRDD` is <<creating-instance, created>> exclusively when `FileSourceScanExec` physical operator is requested to spark-sql-SparkPlan-FileSourceScanExec.md#createBucketedReadRDD[createBucketedReadRDD] or spark-sql-SparkPlan-FileSourceScanExec.md#createNonBucketedReadRDD[createNonBucketedReadRDD] (when `FileSourceScanExec` operator is requested for the spark-sql-SparkPlan-FileSourceScanExec.md#inputRDD[input RDD] when `WholeStageCodegenExec` physical operator is spark-sql-SparkPlan-WholeStageCodegenExec.md#doExecute[executed]).
...
[[files]]
[[index]]
When <<creating-instance, created>>, `FileScanRDD` is given <<filePartitions, FilePartitions>> that are custom RDD partitions with <<spark-sql-PartitionedFile.md#, PartitionedFiles>> (_file blocks_).

`FileScanRDD` uses the following properties when requested to <<compute, compute a partition>>:

* [[ignoreCorruptFiles]] <<spark-sql-properties.md#spark.sql.files.ignoreCorruptFiles, spark.sql.files.ignoreCorruptFiles>>

* [[ignoreMissingFiles]] <<spark-sql-properties.md#spark.sql.files.ignoreMissingFiles, spark.sql.files.ignoreMissingFiles>>

[[creating-instance]]
`FileScanRDD` takes the following to be created:

* [[sparkSession]] SparkSession.md[SparkSession]
* [[readFunction]] Read function that takes a spark-sql-PartitionedFile.md[PartitionedFile] and gives spark-sql-InternalRow.md[internal rows] back (`(PartitionedFile) => Iterator[InternalRow]`)
...
`getPreferredLocations` requests the given <<FilePartition, FilePartition>> (`split`) for <<spark-sql-PartitionedFile.md#, PartitionedFiles>>.
...
NOTE: The given `RDDPartition` is actually a <<FilePartition, FilePartition>> with one or more <<spark-sql-PartitionedFile.md#, PartitionedFiles>> (_file blocks_).
