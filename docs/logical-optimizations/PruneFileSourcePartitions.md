...
`apply` transforms LogicalRelation.md[LogicalRelations] with a [HadoopFsRelation](../datasources/HadoopFsRelation.md) and a [CatalogFileIndex](../datasources/CatalogFileIndex.md) (i.e. for Hive tables) when there are filter expressions defined and the Hive table is partitioned.
...
With predicates left for partition pruning, `apply` requests the [CatalogFileIndex](../datasources/CatalogFileIndex.md) for the [partitions by the predicate expressions](../datasources/CatalogFileIndex.md#filterPartitions) (that gives a [PrunedInMemoryFileIndex](../datasources/PrunedInMemoryFileIndex.md) for a partitioned table).

`apply` replaces the [FileIndex](../datasources/HadoopFsRelation.md#location) in the [HadoopFsRelation](../datasources/HadoopFsRelation.md) with the `PrunedInMemoryFileIndex` and the CatalogStatistics.md#sizeInBytes[total size] statistic to the PartitioningAwareFileIndex.md#sizeInBytes[PrunedInMemoryFileIndex's].
