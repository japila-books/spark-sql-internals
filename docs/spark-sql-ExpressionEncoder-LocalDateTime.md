...
As it is clearly said in the exception, the root cause is no [Encoder](Encoder.md) found for `java.time.LocalDateTime` (as there is not one available in Spark SQL).

You could define one using [ExpressionEncoder](ExpressionEncoder.md), but that does not seem to work either.
...
`LocalDateTimeEncoder` is an _attempt_ to develop a custom [ExpressionEncoder](ExpressionEncoder.md) for Java's [java.time.LocalDateTime]({{ java.api }}/java/time/LocalDateTime.html) so you don't have to map values to another supported type.
