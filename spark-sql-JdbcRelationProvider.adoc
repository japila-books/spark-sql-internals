...
[[shortName]]
`JdbcRelationProvider` is a <<createRelation-CreatableRelationProvider, CreatableRelationProvider>> and a <<createRelation-RelationProvider, RelationProvider>> that is registered to handle *jdbc* data source format.

`JdbcRelationProvider` is a link:spark-sql-DataSourceRegister.adoc[DataSourceRegister] that is registered in `META-INF/services/org.apache.spark.sql.sources.DataSourceRegister` (that you can find in the https://github.com/apache/spark/blob/master/sql/core/src/main/resources/META-INF/services/org.apache.spark.sql.sources.DataSourceRegister[source code] of Apache Spark).
