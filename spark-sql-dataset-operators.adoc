...
Internally, `hint` simply attaches link:spark-sql-LogicalPlan-UnresolvedHint.adoc[UnresolvedHint] unary logical operator to a `Dataset` (i.e. the link:spark-sql-Dataset.adoc#logicalPlan[logical plan] of a `Dataset`).
...
NOTE: `hint` triggers link:spark-sql-QueryExecution.adoc#analyzed[analysis phase] that resolves all hints every time `hint` operator is used.
