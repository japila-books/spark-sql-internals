...
`hint` simply attaches link:spark-sql-LogicalPlan-UnresolvedHint.adoc[UnresolvedHint] unary logical operator to the current link:spark-sql-Dataset.adoc#logicalPlan[logical plan].
...
CAUTION: FIXME Compare `broadcast` function with `hint`
