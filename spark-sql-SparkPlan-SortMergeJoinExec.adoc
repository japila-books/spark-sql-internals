...
The link:spark-sql-SparkPlan.adoc#requiredChildDistribution[partitioning requirements] of the input of a `SortMergeJoinExec` (aka _child output distributions_) are `HashClusteredDistributions` of <<leftKeys, left>> and <<rightKeys, right>> join keys.
