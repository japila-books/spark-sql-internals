...
[NOTE]
====
`HiveTableRelation` is removed from a logical plan when link:HiveAnalysis.adoc[HiveAnalysis] logical rule is executed (and transforms a `InsertIntoTable` with `HiveTableRelation` to an link:spark-sql-LogicalPlan-InsertIntoHiveTable.adoc[InsertIntoHiveTable]).

`HiveTableRelation` is when `RelationConversions` rule is executed (and converts `HiveTableRelations` to `LogicalRelations`).

`HiveTableRelation` is resolved to link:hive/HiveTableScanExec.adoc[HiveTableScanExec] physical operator when link:HiveTableScans.adoc[HiveTableScans] strategy is executed.
====
