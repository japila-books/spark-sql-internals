...
`HiveMetastoreCatalog` is <<creating-instance, created>> exclusively when `HiveSessionStateBuilder` is requested for link:HiveSessionStateBuilder.adoc#catalog[SessionCatalog] (and creates a link:../spark-sql-HiveSessionCatalog.adoc#metastoreCatalog[HiveSessionCatalog]).
