...
| `createRelation`
a| [[createRelation]] Writing the rows of a structured query (a DataFrame) to an external data source

`createRelation` writes the rows of a link:spark-sql-DataFrame.adoc[structured query] to a target relation per save mode and parameters. In the end, `createRelation` creates a link:spark-sql-BaseRelation.adoc[BaseRelation] to describe the relation created.
