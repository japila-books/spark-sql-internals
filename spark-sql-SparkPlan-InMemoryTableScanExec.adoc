...
[[supportCodegen]]
`InMemoryTableScanExec` uses <<supportsBatch, supportsBatch>> flag to control whether link:spark-sql-whole-stage-codegen.adoc[Java code generation] (aka _whole-stage codegen_) is supported or not, i.e. link:spark-sql-CodegenSupport.adoc#supportCodegen[supportCodegen] flag.

[source, scala]
----
// FIXME Example of InMemoryTableScanExec with supportCodegen flag on and off
----
