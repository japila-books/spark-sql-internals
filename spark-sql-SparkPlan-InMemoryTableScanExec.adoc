...
`inputRDD` firstly <<filteredCachedBatches, filteredCachedBatches>> (as a `RDD[CachedBatch]`).
...
=== [[filteredCachedBatches]] Applying Partition Batch Pruning (Creating MapPartitionsRDD of Filtered CachedBatches) -- `filteredCachedBatches` Internal Method
