...
=== [[readHiveStats]] Creating Table Statistics from Hive's Partition Parameters -- `readHiveStats` Internal Method
...
NOTE: `readHiveStats` is used when `HiveClientImpl` is requested for a <<getTableOption, table metadata>> and <<fromHivePartition, table partition metadata>>.
