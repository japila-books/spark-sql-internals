== [[ColumnPruning]] ColumnPruning Logical Query Optimization

`ColumnPruning` is a link:spark-sql-Optimizer.adoc#ColumnPruning[logical optimization] (i.e. `Rule[LogicalPlan]`) that link:spark-sql-Optimizer.adoc[Optimizer] uses to...FIXME

`ColumnPruning` is part of link:spark-sql-Optimizer.adoc#Operator-Optimizations[Operator Optimizations] batch in the base link:spark-sql-Optimizer.adoc[rule-based logical query plan optimizer].
