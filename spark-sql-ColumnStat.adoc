...
`ColumnStat` is <<toMap, converted (serialized) to properties>> while persisting the table (statistics) to a metastore.
...
1. <<rowToColumnStat, rowToColumnStat>>
