== [[RelationConversions]] RelationConversions Logical Evaluation Rule -- Converting Hive Tables
...
1. Transforms a link:spark-sql-LogicalPlan-HiveTableRelation.adoc[HiveTableRelation] with a Hive table (i.e. with `hive` provider) that uses `parquet` or `orc` data storage format
====
