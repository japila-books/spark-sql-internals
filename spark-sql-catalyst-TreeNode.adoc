...
`TreeNode` is a node in <<spark-sql-catalyst.adoc#, Catalyst>> tree with zero or more <<children, children>>.

`TreeNode` is a recursive data structure that holds itself only or other `TreeNodes` which is used to build trees (e.g. of <<spark-sql-Expression.adoc#, Catalyst expressions>> or operators in <<spark-sql-catalyst-QueryPlan.adoc#, query plan>>).
