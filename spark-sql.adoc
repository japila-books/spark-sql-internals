...
Spark SQL supports structured queries in *batch* and *streaming* modes (with the latter as a separate module of Spark SQL called link:spark-structured-streaming.adoc[Spark Structured Streaming]).
...
As of Spark 2.0, Spark SQL is now _de facto_ the primary and feature-rich interface to Spark's underlying in-memory distributed platform (hiding Spark Core's RDDs behind higher-level abstractions). In other words, you use Spark SQL's `Dataset` API to describe a distributed computation that eventually be converted to a link:spark-sql-QueryExecution.adoc#toRdd[DAG of RDDs].
