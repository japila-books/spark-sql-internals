...
Regardless of the query language, i.e. SQL or Dataset API, structured queries in Spark SQL all end up as a link:spark-sql-catalyst.adoc[Catalyst tree] of link:spark-sql-catalyst-QueryPlan.adoc[relational operators] and link:spark-sql-Expression.adoc[expressions]. When executed, they go through the following link:spark-sql-QueryExecution.adoc#execution-pipeline[execution stages] (aka _Structured Query Execution Pipeline_):
...
1. link:spark-sql-QueryExecution.adoc#optimizedPlan[Logical Optimization] (using link:spark-sql-SparkOptimizer.adoc[rule-based] or link:spark-sql-cost-based-optimization.adoc[cost-based] optimizations)
1. link:spark-sql-QueryExecution.adoc#sparkPlan[Physical Planning]
1. link:spark-sql-QueryExecution.adoc#executedPlan[Physical Optimization] (e.g. link:spark-sql-whole-stage-codegen.adoc[Whole-Stage Java Code Generation])
