...
NOTE: `View` is resolved by <<spark-sql-ResolveRelations.adoc#, ResolveRelations>> logical resolution.

NOTE: <<spark-sql-AliasViewChild.adoc#, AliasViewChild>> logical analysis rule makes sure that the <<output, output>> of a `View` matches the output of the <<child, child>> logical operator.
