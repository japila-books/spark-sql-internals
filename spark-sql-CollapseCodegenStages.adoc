...
Internally, `insertWholeStageCodegen` walks down the tree (recursively) to find <<spark-sql-SparkPlan.adoc#, physical operators>> with <<spark-sql-CodegenSupport.adoc#, CodegenSupport>> and meet the <<supportCodegen, supportCodegen>> requirements.
