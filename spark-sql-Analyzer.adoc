...
`Analyzer` is the *logical query plan analyzer* (aka _Spark Analyzer_ or _Query Analyzer_) that <<execute, semantically validates and transforms an unresolved logical plan>> to an *analyzed logical plan*.
