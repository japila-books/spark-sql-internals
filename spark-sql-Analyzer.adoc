...
| Checks whether a function identifier (referenced by an link:spark-sql-Expression-UnresolvedFunction.adoc[UnresolvedFunction]) link:spark-sql-SessionCatalog.adoc#functionExists[exists in the function registry]. Throws a `NoSuchFunctionException` if not.
...
| ResolveGenerate
|
| [[ResolveFunctions]] link:spark-sql-ResolveFunctions.adoc[ResolveFunctions]
