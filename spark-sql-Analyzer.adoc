...
| Adds a link:spark-sql-LogicalPlan-ResolvedHint.adoc[ResolvedHint] unary operator to a logical plan for `BROADCAST`, `BROADCASTJOIN` and `MAPJOIN` hint names (and no parameters) in a link:spark-sql-LogicalPlan-UnresolvedHint.adoc[UnresolvedHint] operator.
...
| Resolves link:spark-sql-LogicalPlan-InsertIntoTable.adoc[InsertIntoTable] and link:spark-sql-LogicalPlan-UnresolvedRelation.adoc[UnresolvedRelation] operators
