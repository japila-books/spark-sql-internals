...
=== [[getKafkaOffsetRangeLimit]] Getting Desired KafkaOffsetRangeLimit -- `getKafkaOffsetRangeLimit` Object Method
...
* `KafkaSourceProvider` is requested to <<validateBatchOptions, validate Kafka options>> and <<createRelation-RelationProvider, create a BaseRelation>> (as a <<spark-sql-RelationProvider.adoc#, RelationProvider>>)

* (Spark Structured Streaming) `createSource` and `createContinuousReader`
====
