...
| spark.sql.debug
a| [[spark.sql.debug]][[DEBUG_MODE]] *(internal)* Only used for internal debugging when `HiveExternalCatalog` is requested to <<spark-sql-HiveExternalCatalog.adoc#restoreTableMetadata, restoreTableMetadata>>.
