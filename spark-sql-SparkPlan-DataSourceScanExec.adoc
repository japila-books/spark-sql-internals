...
[[nodeNamePrefix]]
The default *node name prefix* is an empty string (that is used at the very beginning of <<simpleString, simple text node description>>).
...
| [[metadata]] `metadata`
| Metadata (as a collection of key-value pairs) that describes a scan.

Used for <<simpleString, simple text representation>>
