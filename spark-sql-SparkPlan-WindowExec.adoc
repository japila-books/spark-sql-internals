...
* <<spark-sql-AllTuples.adoc#, AllTuples>> when the <<partitionSpec, window partition specification expressions>> is empty

* <<spark-sql-ClusteredDistribution.adoc#, ClusteredDistribution>> (with the <<partitionSpec, window partition specification expressions>>) with the <<partitionSpec, partition specification>> specified
