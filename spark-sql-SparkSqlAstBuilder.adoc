...
* [[DescribeColumnCommand]] link:spark-sql-LogicalPlan-RunnableCommand.adoc#DescribeColumnCommand[DescribeColumnCommand] logical command for `DESCRIBE TABLE` with a column only (i.e. no `PARTITION` specification).
+
[source, scala]
----
// Seq((0, 0, "zero"), (1, 1, "one")).toDF("id", "p1", "p2").write.partitionBy("p1", "p2").saveAsTable("t1")
...
* [[DescribeTableCommand]] link:spark-sql-LogicalPlan-RunnableCommand.adoc#DescribeTableCommand[DescribeTableCommand] logical command for all other variants of `DESCRIBE TABLE` (i.e. no column)
+
[source, scala]
----
// Seq((0, 0, "zero"), (1, 1, "one")).toDF("id", "p1", "p2").write.partitionBy("p1", "p2").saveAsTable("t1")
