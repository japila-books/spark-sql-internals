...
* `AnalyzePartitionCommand` logical command for `ANALYZE TABLE` with `PARTITION` specification and no `FOR COLUMNS` clause
+
```
// Seq((0, 0, "zero"), (1, 1, "one")).toDF("id", "p1", "p2").write.partitionBy("p1", "p2").saveAsTable("t1")
val sqlText = "ANALYZE TABLE t1 PARTITION (p1, p2) COMPUTE STATISTICS"
...
* `AnalyzeTableCommand` logical command for `ANALYZE TABLE` with no `PARTITION` specification and `FOR COLUMNS` clauses
+
```
// Seq((0, 0, "zero"), (1, 1, "one")).toDF("id", "p1", "p2").write.partitionBy("p1", "p2").saveAsTable("t1")
...
* `AnalyzeColumnCommand` logical command for `ANALYZE TABLE` with `FOR COLUMNS` clause and no `PARTITION` specification
+
```
// Seq((0, 0, "zero"), (1, 1, "one")).toDF("id", "p1", "p2").write.partitionBy("p1", "p2").saveAsTable("t1")
